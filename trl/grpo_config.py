...
class GRPOConfig(object):
    ...
    steps: Optional[int] = field(default=20000, metadata={
    ...
    vf_coef: Optional[float] = field(
        default=0.005, metadata={"help": "Scaling factor for value loss"})
    ...
    forward_batch_size: Optional[int] = field(
        default=None,
        metadata={
            "help": "Number of samples forward passed through model at a time"},
    )
    # mini_batch_size: Optional[int] = field(
    #     default=1, metadata={"help": "Number of samples optimized inside GRPO together"}
    # )
    ...
    project_kwargs: Optional[dict] = field(
        ...
        metadata={
            "help": "Keyword arguments for the accelerator project config (e.g. `logging_dir`)"},
    )
    ...
    early_stopping: Optional[bool] = field(
        default=False, metadata={"help": "Whether to stop the GRPO optimization loop early is the KL too high"}
    )
    ...
    ratio_threshold: Optional[float] = field(
        default=20.0, metadata={"help": "Skip mini-batches with high GRPO ratios that can cause loss spikes"}
    )
