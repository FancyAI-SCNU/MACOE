...
class TeacherObs(RuleObs):
    ...
    def get_obs(
        self,raw_df, feature_dfs, t, interval, position, target, is_buy, max_step_num, interval_num,agent_ix, *args, **kargs,
    ):
        if t == -1:
            if agent_ix == 0:
                self.private_states = []
                self.private_states.append([])
            else:
                self.private_states.append([])
        ...
        self.private_states[agent_ix].append(private_state)
        list_private_state = np.concatenate(self.private_states[agent_ix])
        
        list_private_state = np.concatenate(
            (list_private_state, [0.0] * 2 * (interval_num + 1 - len(self.private_states[agent_ix])),)
        )
